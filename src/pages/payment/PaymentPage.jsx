...
      });
      ...
      // ✅ FIX 1: Correctly destructure 'paymentId' (as returned by the backend)
      const { orderId, amount, currency, paymentId } = data; 

      const options = {
        key: "rzp_live_RQrrfePov3rc0O", // Replace with your actual live key later
        amount: amount.toString(),
        currency,
        name: "Retrowoods Furnitures",
...
              ...response,
              // ✅ FIX 2: Send 'paymentId' (the variable captured above)
              paymentId, 
            });
...
                "successBooking",
                JSON.stringify(verifyRes.data.booking) 
              );
...
      localStorage.setItem(
        "successBooking",
        JSON.stringify(res.data.booking) // Use the final booking object from the backend
      );
      localStorage.removeItem("cart");
      localStorage.removeItem("pendingBooking");
      await clearCartBackend();
...
          {/* LEFT SIDE — ORDER SUMMARY (UI unchanged) */}
...
          {/* RIGHT SIDE — CUSTOMER & PAYMENT INFO (UI unchanged) */}
